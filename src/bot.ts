...
import { removeById } from "./commands/remove";
...
const commands = [
  {
    name: "add",
    description: "Add user to Marvel Rivals Leaderboard!",
    options: [
      {
        name: "id",
  ...
  {
    name: "remove",
    description: "Remove user from Marvel Rivals Leaderboard!",
    options: [
      {
        name: "id",
  ...
  {
    name: "rank",
    description: "Show the Marvel Rivals leaderboards",
  },
];
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.commandName === "add") {
    ...
    console.log("Adding ID...", `${id} - ${name}`);
    await interaction.deferReply();
    if (id && id.length < 30) {
      if (!id.match(/^[0-9]+$/)) {
        await interaction.editReply(
          "Please provide a valid ID. (ID should be only numbers)"
        );
      } else if (!(await checkIfPlayerExist(id))) {
        const url = `https://rivalsmeta.com/player/${id}`;
        await interaction.editReply(
  ...
  if (interaction.commandName === "remove") {
    const id = decodeURI(interaction.options.getString("name") || "");
